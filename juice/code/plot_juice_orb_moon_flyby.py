...
rm = 1737.4  # km 月半径
re = 6378.1  # km 地球半径
ionospheric_thick = 40.0  # km 電離圏の厚さ
...
start_ionospheric_occulataion_ind_arr = np.zeros(len(radio_x))
end_ionospheric_occulataion_ind_arr = np.zeros(len(radio_x))

for j in range(len(radio_x)):
    tangential_dis = np.zeros(len(x_j))
    tangential_dis_abs = np.zeros(len(x_j))
    tangential_dis_abs_ionopshre = np.zeros(len(x_j))
    for i in range(len(x_j)):
        ...
        tangential_dis_abs_ionopshre[i] = abs(vn - rm - ionospheric_thick)
    ...
    if tangential_dis[closest_ind] < 0:
        ...
        start_ionospheric_occulataion_ind_arr[j] = np.argmin(
            tangential_dis_abs_ionopshre[:closest_ind]
        )
        end_ionospheric_occulataion_ind_arr[j] = closest_ind + np.argmin(
            tangential_dis_abs_ionopshre[closest_ind:]
        )
        ...
        start_ionospheric_occulataion_ind_arr[j] = np.nan
        end_ionospheric_occulataion_ind_arr[j] = np.nan

start_penumbra_ind = int(np.min(start_occulataion_ind_arr))
start_umbra_ind = int(np.max(start_occulataion_ind_arr))
start_ionospheric_umbra_ind = int(np.max(start_ionospheric_occulataion_ind_arr))
end_umbra_ind = int(np.min(end_occulataion_ind_arr))
end_penumbra_ind = int(np.max(end_occulataion_ind_arr))
end_ionospheric_umbra_ind = int(np.min(end_ionospheric_occulataion_ind_arr))
...
plt.axis("equal")
plt.savefig(save_folder_path + type + "moon_flyby_xy.png")
plt.show()
...
plt.axis("equal")
plt.savefig(save_folder_path + type + "moon_flyby_xz.png")
plt.show()
...
plt.xticks(x_values, x_labels)
plt.savefig(save_folder_path + type + "moon_flyby_tr.png")
plt.show()
...
plt.scatter(
    x_j_m[start_ionospheric_umbra_ind] / rm,
    y_j_m[start_ionospheric_umbra_ind] / rm,
    c="orangered",
)
plt.text(
    x_j_m[start_ionospheric_umbra_ind] / rm,
    y_j_m[start_ionospheric_umbra_ind] / rm,
    "Ionospheric umbra"
    + str(spice.et2datetime(et[start_ionospheric_umbra_ind]).hour)
    + ":"
    + str(spice.et2datetime(et[start_ionospheric_umbra_ind]).minute)
    + ":"
    + str(spice.et2datetime(et[start_ionospheric_umbra_ind]).second),
    c="orangered",
)
...
plt.scatter(
    x_j_m[end_ionospheric_umbra_ind] / rm,
    y_j_m[end_ionospheric_umbra_ind] / rm,
    c="orangered",
)
plt.text(
    x_j_m[end_ionospheric_umbra_ind] / rm,
    y_j_m[end_ionospheric_umbra_ind] / rm,
    "Ionospheric numbra "
    + str(spice.et2datetime(et[end_ionospheric_umbra_ind]).hour)
    + ":"
    + str(spice.et2datetime(et[end_ionospheric_umbra_ind]).minute)
    + ":"
    + str(spice.et2datetime(et[end_ionospheric_umbra_ind]).second),
    c="orangered",
)
...
plt.axis("equal")
plt.savefig(save_folder_path + type + "moon_flyby_xy_ionosphere.png")
plt.show()
