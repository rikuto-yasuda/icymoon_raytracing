...
bool ray::intermediate::operator==(
	const ray::intermediate &r) const
{
	// 高速化のため、&& ではなく'&'を使う。
	...
	const vector &k) const
{
	...
	const double theta = std::acos(
		inner_prod(i.B / norm_2(i.B), k / norm_2(k))); // theta = 磁場と成す角
	i.s = std::sin(theta);
	i.c = std::cos(theta);
	i.s2 = i.s * i.s;
	i.c2 = i.c * i.c;
	...
	i.root = std::sqrt(
		i.Y4 * i.s2 * i.s2 + 4.0 * i.Y2 * i.c2 * i.iX22);
	...
	const vector &k)
{
	...
	const vector k_new = k * std::sqrt(1 - i.numerator / i.denominator) * m_wave.getFreq() / (cnst::c * norm_2(k));
	...
	const vector &k) const
{
	...
	const double
		...
		ddG_dw = clearNaN(
			4 * (i.X2 / i.w) + 2 * (i.Y2 / i.w) * i.s2 + ox * 2 * (-(i.Y4 / i.w) * (i.s2 * i.s2) - 2 * (i.Y2 / i.w) * i.c2 * (i.iX22) + 4 * (i.X2 * i.Y2 / i.w) * i.c2 * i.iX2) / i.root);

	return clearNaN(
		first_term + (i.denominator * dnG_dw - i.numerator * ddG_dw) / (i.denominator * i.denominator));
}
	...
	const vector &k) const
{
	...
	for (int n = 0; n < 3; ++n)
	{
		...
		const double
			dc2 = clearNaN(
				2. * i.Bk / (i.B2 * i.B2 * i.k2) * (i.B2 * kdBdr - i.Bk * BdBdr)),
			ds2 = -dc2,
		...
		const double
			...
			second_term = clearNaN(
				-2 * (dwp2w2)-i.s2 * (dwc2w2) - (i.Y2) * ds2 + ox * (2 * (i.s2 * i.s2) * (i.Y2) * dwc2w2 + (i.Y4) * ds4 + 4 * i.c2 * i.iX22 * dwc2w2 + 4 * (i.Y2) * (i.iX22) * dc2 - 8 * (i.Y2) * (i.c2) * (i.iX2) * (dwp2w2)) / (2 * i.root));

		result(n) = clearNaN(
			(i.denominator * first_term - i.numerator * second_term) / (i.denominator * i.denominator));
	}
	return result;
}
	...
	const vector &k) const
{
	...
	for (int n = 0; n < 3; ++n)
	{
		const double
			dc2dk = clearNaN(
				2 * (i.Bk) / (i.B2 * i.k2 * i.k2) * (i.B(n) * i.k2 - i.Bk * k(n))),
			ds2dk = -dc2dk,
			...
			second_term = clearNaN(
				-(i.Y2) * ds2dk + ox * (i.Y4 * ds4dk + 4 * i.Y2 * i.iX22 * dc2dk) / (2 * i.root));
