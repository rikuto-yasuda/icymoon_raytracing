...
namespace rtc { namespace model { namespace plasma {
	...
	*********************************************/
	...
	public:
		...
		class parameter : public EUROPA_PARAM
		{
		public:
			parameter(){ MLT = iLAT = N0 = z0 = H0 = beta = 0.0; };
			parameter( const EUROPA_PARAM& p );

			parameter& operator += ( const EUROPA_PARAM& p );
			parameter& operator =  ( const parameter& p );

			parameter operator +( const EUROPA_PARAM& p ) const;
	...
	private:
		...
		const static EUROPA_PARAM m_baseParam[];
