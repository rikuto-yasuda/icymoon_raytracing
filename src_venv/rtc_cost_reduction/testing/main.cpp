...
int raytrace_start(testing_env *env)
{
	...
	if (env->is_parallel)
	{
		...
		for (int parallel = 0; parallel < env->parallel_number; ++parallel)
		{
			// raytraceクラスを構築し、operator ()を呼び出してループ処理を行う。
			// 光が終着点に到達したとき、operator ()から制御が返る。
			//raytrace rtrc(env, (2 * rtc::cnst::pi / env->parallel_number) * parallel, 0.0);
			raytrace rtrc(env, 0.0, 100e3 * parallel); 
	...
	else
	{
		...
		for (int parallel = 0; parallel < env->parallel_number; ++parallel)
		{
			// raytrace *r = new raytrace(env, (2 * rtc::cnst::pi / env->parallel_number) * parallel, 0.0);
			raytrace *r = new raytrace(env, 0.0, 100e3 * parallel);	
			rays.push_back(r);
