    ...
    if detectable_obsever_position_z > 0 or detectable_obsever_position_x < 0:  # z座標が0以下のものは受かる可能性がレイパスを見る必要もなし
        ...
        for j in range(Lowest[freq], Highest[freq], 2):
            ...
            if Radio_propagation_route.ndim == 2:
                ...
                detectable_z_index_minus = detectable_z_index - 1

                #　レイが探査機のx座標を超える前に探査機よりも上空に行ってしまっている→その電波は受からない
                if len(np.intersect1d(detectable_x_index, detectable_z_index_minus)) == 0:
                    continue
                ...
                if len(np.intersect1d(detectable_x_index, detectable_z_index_minus)) > 1:
                    print("error")

                else:
                    print("enter")
                    h = int(np.intersect1d(detectable_x_index,
                            detectable_z_index_minus)[0])+1
