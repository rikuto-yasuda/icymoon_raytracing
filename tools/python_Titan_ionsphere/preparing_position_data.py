...
def Interpolate_radio_source_and_save(
    spacecraft_ephemeris_csv, moon_ephemeris_csv, time
):
    ...
    radio_source_position = np.loadtxt(
        result_data_path + "expres_detectable_radio_data_of_each_flyby/All_"
        + spacecraft_name
        + "_"
        + object_name
        + "_"
...
def Spacecraft_ephemeris_calc(spacecraft_ephemeris_data, moon_ephemeris_data, time):
...
def Save_data(data):
    np.savetxt(
        result_data_path + "calculated_expres_detectable_radio_data_of_each_flyby/interpolated_calculated_all_"
        + spacecraft_name
        + "_"
        + object_name
        + "_"
...
def main():
    ...
    # 探査機の位置データを補間する
    Interpolate_radio_source_and_save(spacecraft_epemeris, moon_epemeris, time)
    ...
    res, Xmax, Xmin, Zmax, Zmin  = Spacecraft_ephemeris_calc(
        interpolated_spacecraft_ephemeris, interpolated_moon_ephemeris, time
    )
    Save_data(res)
