# %%
# evaluate_ft_2.pyからevaluate_ft_2_revision2.pyへ、偏波ごとに電波源タイプを変える方向で更新
# evaluate_ft_2_revision2.pyからevaluate_ft_2_revision3.pyへ、電波源タイプごとに別密度でフィッティングする方向で更新
import numpy as np
...
LH_radio_type_A2D = "D"  # saturn radioの場合'C' or 'D'
...
def get_frequency_intensity_plotparameter(
    moon_name, flyby_time, ingress_or_egerss
):
...
def plot_difference(maxdens, peakheight, sigma, RH_radio_type, LH_radio_type, using_frequency_array ,using_frequency_range):
    # [[frequencyの配列] [time_lagの配列]]
    
    # 右回り電波の電波源を仮定した時差データを取得
    RH_time_difference_data = np.loadtxt(
    ...
    # 左回り偏頗の電波源を仮定した時差データを取得
    LH_time_difference_data = np.loadtxt(
            "../../result_titan/radio_raytracing_occultation_timing_def_"
            + spacecraft_name
            + "_"
            + object_name
    ...
    galileo_frequency_array = RH_time_difference_data[0]
    RH_time_difference_array = RH_time_difference_data[1]
    LH_time_difference_array = LH_time_difference_data[1]
    polarization_data_array = RH_time_difference_data[2]

    LH_total_time_difference = 0
    LH_total_frequency_number = 0
    
    RH_total_time_difference = 0
    RH_total_frequency_number = 0
    ...
    for i in output_freq_position:
        ...
        polarization_icon = polarization_data_array[closest_index]
        
        if polarization_icon == 1:
            LH_total_time_difference += LH_time_difference_array[closest_index]
            LH_total_frequency_number += 1
        
        elif polarization_icon == -1:
            RH_total_time_difference += RH_time_difference_array[closest_index]
            RH_total_frequency_number += 1

        elif polarization_icon == 0:
            print("polarization icon is 0")     

        if max_frequency_difference < frequency_difference:
            max_frequency_difference = frequency_difference
    
    LH_average_difference_time = LH_total_time_difference / LH_total_frequency_number
    RH_average_difference_time = RH_total_time_difference / RH_total_frequency_number
    ...
    LH_time_dif.append(float(LH_average_difference_time))
    RH_time_dif.append(float(RH_average_difference_time))

    return LH_total_frequency_number, RH_total_frequency_number


def LH_fig_and_save_def(
    def_data,
    frequency_range,
    holizontal_size,
    vertical_size,
    ...
    best_time_difference = def_data[np.argmin(def_data[:,3]),3]

    print(LH_radio_type_A2D)
    print("best_max_density", best_max_density)
    print("best_peak_height", best_peak_height)
    print("best_sig", best_sig)
    print("best_time_difference", best_time_difference)
    ...
    return best_max_density, best_peak_height, best_sig, best_time_difference

def RH_fig_and_save_def(
    def_data,
    frequency_range,
    holizontal_size,
    vertical_size,
...
def main():
    (
        ...
        object_name, time_of_flybies, occultaion_type
    )
    ...
    for file in use_files:
        ...
        LH_freq_num, RH_freq_num = plot_difference(
            highest_density_str,
    ...
    ### ここから結果の出力
    LH_output_array = np.array(max + peakh + sig+ LH_time_dif)
    LH_output_array = LH_output_array.reshape(4, int(len(LH_output_array) / 4)).T
    print(LH_output_array)

    RH_output_array = np.array(max + peakh + sig+ RH_time_dif)
    RH_output_array = RH_output_array.reshape(4, int(len(RH_output_array) / 4)).T
    print(RH_output_array)

    
    LH_best_max_density, LH_best_peak_height, LH_best_sig, LH_best_time_difference = LH_fig_and_save_def(
        LH_output_array,
        using_frequency_range,
        fig_holizontal_size,
        fig_vertical_size,
    ...
    RH_best_max_density, RH_best_peak_height, RH_best_sig, RH_best_time_difference = RH_fig_and_save_def(
        RH_output_array,
        using_frequency_range,
        fig_holizontal_size,
        fig_vertical_size,
    ...
    min_average = ((LH_best_time_difference * LH_freq_num) + (RH_best_time_difference * RH_freq_num)) / (LH_freq_num + RH_freq_num)
    print("total_min_average", min_average)
    print(LH_freq_num, RH_freq_num)
