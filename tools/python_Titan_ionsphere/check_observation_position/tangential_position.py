...
object_name = "titan"  # europa/ganymde/callisto
...
source_file = "/Users/yasudarikuto/research/icymoon_raytracing/tools/result_titan/"
begining_ingress_hour = 9
begining_ingress_minute = 15

end_ingress_hour = 9
end_ingress_minute = 55

lowest_frequency_ingress = 0.1
highest_frequecy_ingress = 0.8

radio_type_ingress = "A"  # 複数選択可能にしたい

begining_egress_hour = 9
begining_egress_minute = 40

end_egress_hour = 10
end_egress_minute = 30

lowest_frequency_egress = 0.1
highest_frequecy_egress = 0.8

radio_type_egress = "A"  # 複数選択可能にしたい
...
def Pick_up_time():
    flyby_list_path = source_file + "occultation_flyby_list.csv"
...
def Pick_up_spacecraft_csv():
    ...
    spacecraft_csv_path = source_file + "spacecraft_ephemeris/" + csv_name
...
def Pick_up_spacecraft_csv_coordinate():
    ...
    spacecraft_csv_path = (
        source_file + "ephemeris_for_coordinate_transformation/spacecraft/"
        + csv_name
    )
    ...
    #print(spacecraft_ephemeris_coordinate_csv)
...
def Pick_up_moon_csv():
    ...
    spacecraft_csv_path = source_file + "moon_ephemeris/" + csv_name
    spacecraft_ephemeris_csv = pd.read_csv(
        spacecraft_csv_path, header=15, skipfooter=4
    )  # ヘッダーの位置と末端のデータの位置をheaderとskipfooterで設定
...
def Pick_up_moon_csv_coordinate():
    ...
    moon_csv_path = (
        source_file + "ephemeris_for_coordinate_transformation/moon/"
        + csv_name
    )
    ...
    return (
        moon_ephemeris_coordinate_csv, time_information
    )  # IAU_"moon"で考えた木星から見た月の位置
...
def Spacecraft_ephemeris_calc(
    ...
    radio_source_position = np.loadtxt(
        source_file
        + "expres_detectable_radio_data_of_each_flyby/All_"
        + spacecraft_name
        + "_"
        + object_name
        + "_"
    ...
    radio_source_position[:, 12:15] = radio_source_position[:, 12:15] * 58232
...
def Save_data(data):
    np.savetxt(
        source_file
        + "/calculated_expres_detectable_radio_data_of_each_flyby/calculated_all_"
        + spacecraft_name
        + "_"
        + object_name
        + "_"
...
def Occultation_timing_select(row_data, time_data):
    ...
    def lon_and_lat(
        ...
        # 出力する周波数chの値と数を取得
        Freq_list_indices = np.where((np.array(Freq_num) > lowest_freq) & (np.array(Freq_num) < highest_freq))[0]
        selected_freq_values = np.array(Freq_num)[Freq_list_indices]
        Freq_ch_num = len(Freq_list_indices)

        empty_arrays = [[] for _ in range(Freq_ch_num)]

        for i in range(len(selected_detectable_data)):
            # 周波数chの値を取得
            Freq_ch = np.where(
                (selected_freq_values < (selected_detectable_data[i][2] + 0.00001)) &
                (selected_freq_values > (selected_detectable_data[i][2] - 0.00001))
            )[0][0]

            # 周波数chごとにデータを格納
            empty_arrays[Freq_ch].append(selected_detectable_data[i])
        

        lon_array = np.zeros(Freq_ch_num)
        lat_array = np.zeros(Freq_ch_num)
        z_array = np.zeros(Freq_ch_num)
        for i in range(Freq_ch_num):
            data = np.array(empty_arrays[i]) 
            z_array = data[:, 11]
            z_array_min_index = np.argmin(z_array)

            lon = data[z_array_min_index, 5]  # tangential pointの経度
            lat = data[z_array_min_index, 6]  # tangential pointの緯度
            lon_array[i] = lon
            lat_array[i] = lat
            z_array[i] = data[z_array_min_index, 11]  # tangential pointからの探査機方向に伸ばした時の衛星高度
        
        if min(lon_array) < -90 and max(lon_array) > 90:
        ...
        #print(z_array)
        print("average z:   " + str(np.mean(z_array)))
        print("average lon: " + str(mean_lon))
        print("average lat: " + str(np.mean(lat_array)))

    print(radio_type_ingress)
    print("ingress")
...
def main():
    ...
    moon_epemeris, time_moon = Pick_up_moon_csv()  # 月の位置データとフライバイリストから持ってきた時刻データを出力
