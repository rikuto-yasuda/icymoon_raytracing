...
path = "/Users/yasudarikuto/research/icymoon_raytracing/tools/result_titan/previous_study_ephemeris_revised/"
earth_csv_name = "WGC_StateVector_20250805134542.csv"
saturn_csv_name = "WGC_StateVector_20250805134505.csv"
sun_csv_name = "WGC_StateVector_20250805133840.csv"
spacecraft_csv_name = "WGC_StateVector_20250805134615.csv"


# ヘッダーの位置と末端のデータの位置をheaderとskipfooterで設定
saturn_csv = pd.read_csv(
    path
    + "saturn/"
    + saturn_csv_name,
    header=15,
...
def check_time(csv_1, csv_2, csv_3, csv_4):
    ...
    Csv_3 = np.array(csv_3["UTC calendar date"])  # saturn
...
def calc_SZA(sun_ephemeris, spacecraft_latitude_deg, spacecraft_longitude_deg):
    ...
    naiseki_calced = naiseki_calc(
        sun_latitude_rad,
        sun_longitude_rad,
        spacecraft_latitude_rad,
        spacecraft_longitude_rad,
    )
...
def calc_Ram(jupiter_ephemeris, spacecraft_latitude_deg, spacecraft_longitude_deg):
    ...
    trailing center longitude : saturn longitude + 90 deg
    Args:
        sun_ephemeris (_type_): _description_
        spacecraft_ephemeris (_type_): _description_
    ...
    jupiter_trailing_longitude_deg = np.array(jupiter_ephemeris["Longitude (deg)"]) + 90

    jupiter_trailing_latitude_deg = np.zeros(
        len(jupiter_ephemeris["Latitude (deg)"])
    )  # trailing center latitude = 0
    ...
    naiseki_calced = naiseki_calc(
        jupiter_latitude_rad,
        jupiter_longitude_rad,
        spacecraft_latitude_rad,
        spacecraft_longitude_rad,
    )
...
def calc_Psi(jupiter_ephemeris, sun_ephemeris):
    ...
    jupiter_trailing_longitude_rad = np.radians(jupiter_trailing_longitude_deg)  # 経度Q1
    ...
    naiseki_calced = naiseki_calc(
        jupiter_latitude_rad,
        jupiter_trailing_longitude_rad,
        sun_latitude_rad,
        sun_longitude_rad,
    )
...
def Earth_jupiter_tangetial_point(spacecraft_ephemeris, earth_ephemeris):
    ...
    spacecraft_longitude_deg = Float_64(
        np.array(spacecraft_ephemeris["Longitude (deg)"])
    )
    spacecraft_latitude_deg = Float_64(np.array(spacecraft_ephemeris["Latitude (deg)"]))
    spacecraft_radius_km = Float_64(np.array(spacecraft_ephemeris["Radius (km)"]))

    earth_longitude_deg = Float_64(np.array(earth_ephemeris["Longitude (deg)"]))
    ...
    earth_x, earth_y, earth_z = Kyoku_2_Choku(earth_longitude_rad, earth_latitude_rad)
    ...
    for j in range(0, n):
        ...
        vertical_unit_vector = (
            vertical_vector / vertical_vector_length
        )  # 探査機方向と地球方向のベクトルに垂直な外積
        tangential_point = np.cross(
            earth_vector, vertical_unit_vector
        )  # 掩蔽地点の座標をxyz座標で

        tangential_point_length = np.linalg.norm(tangential_point)
        tangential_unit_vector = (
            tangential_point / tangential_point_length
        )  # 掩蔽地点の方向を表す単位ベクトル　xyz

        tangential_lat[j] = math.asin(tangential_unit_vector[2]) * 180.0 / math.pi
        tangential_lon[j] = (
            math.atan2(tangential_unit_vector[1], tangential_unit_vector[0])
            * 180.0
            / math.pi
        )
...
def save_all_result(
    ...
    df2["Spacecraft_latitude(deg)"] = np.array(spacecraft_ephemeris["Latitude (deg)"])
...
def main():
    check_time(sun_csv, spacecraft_csv, saturn_csv, earth_csv)
    ...
    Ram_spacecraft = calc_Ram(
        saturn_csv, spacecraft_latitude_deg, spacecraft_longitude_deg
    )

    Psi = calc_Psi(saturn_csv, sun_csv)
    ...
    Ram_occult = calc_Ram(saturn_csv, Occult_lat, Occult_lon)
