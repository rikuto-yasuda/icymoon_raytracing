...
maximum_subsolar_long = 198.78515960999994
minimum_subsolar_long = 196.08295604
...
def Prepare_Figure(judgement, time_information):
    ...
    hour_select1 = np.where(
        (selected_data[:, 0] == 11) & (selected_data[:, 1] > 48))

    hour_select2 = np.where(
        (selected_data[:, 0] == 12) & (selected_data[:, 1] < 5))

    hour_select = np.hstack((hour_select1, hour_select2))
    print(hour_select)
    """
    hour_select = np.where(selected_data[:, 0] == 11)

    selected_hour_data = (selected_data[hour_select, :])[0]

    minute_select = np.where(
        (21 < selected_hour_data[:, 1]) & (selected_hour_data[:, 1] < 26))

    selected_time_data = (selected_hour_data[minute_select, :])[0]
    print(selected_time_data)
    # using_frequency_range = [5.5e-1, 6]
    # C30 egress [6e-1, 6] Egress
    freq_select = np.where(
        (6e-1 < selected_time_data[:, 2]) & (selected_time_data[:, 2] < 6))
