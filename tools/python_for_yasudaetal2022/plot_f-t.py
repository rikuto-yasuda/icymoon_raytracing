...
def Make_FT_full(
    ...
    for k in range(len(galileo_data_freq)):
        ...
        over_judge_time_list = np.array(
            np.where((galileo_radio_intensity[k][:] > boundary_intensity))
        )
    ...
    def plot_and_save(start_time, end_time, name):
        ...
        plt.annotate(
            str(vertical_line_freq) + "MHz",
            (start_time + 20, vertical_line_freq + 0.05),
            color="hotpink",
        )
        ax.set_title(
            "Maximum density "
...
def main():
    ...
    for i in range(4):
        evaluated_data = Evaluate_raytrace_data(detectable_data[i], time_information)
