...
def Spacecraft_ephemeris_calc(spacecraft_ephemeris_csv, moon_ephemeris_csv, time):
    ...
    for i in range(radio_source_data_length):
        ...
        res[i][9] = math.degrees(math.atan2(
        ...
        longtitude_ey = np.degrees(np.arctan2(ey[0], ey[1]))

        er = r2 / np.linalg.norm(r2)  # 木星からガニメデ表面への単位ベクトル
        longtitude_er = np.degrees(np.arctan2(er[0], er[1]))
        ...
        longtitude_ex = np.degrees(np.arctan2(ex[0], ex[1]))

        longtitude_x = (longtitude_ex - longtitude_er-180) * (-1)
        if longtitude_x < 0:
            longtitude_x += 360
...
def Save_data(data):
    np.savetxt('../result_for_yasudaetal2022/calculated_expres_detectable_radio_data_of_each_flyby/calculated_all_' +
               spacecraft_name+'_'+object_name+'_'+str(time_of_flybies)+'_tangential_point.txt', data, fmt="%s")
