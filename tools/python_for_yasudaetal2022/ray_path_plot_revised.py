...
def Occultation_timing_select(row_data):
    ...
    data = np.loadtxt(
        '../result_for_yasudaetal2022/calculated_expres_detectable_radio_data_of_each_flyby/calculated_all_'+spacecraft_name+'_'+object_name+'_'+str(time_of_flybies)+'_Radio_data.txt')
    ...
    def time_select_and_xz(radio_data, begining_hour, begining_min, end_hour, end_min, lowest_freq, highest_freq):
    ...
    h, m, x, z = time_select_and_xz(source_type_selected, occultation_begining_hour, occultation_begining_minute,
                                    occultation_end_hour, occultation_end_minute, occultation_lowest_frequency, occultation_highest_frequecy)
    print(x, z)
    return h, m, x, z
...
def spacecraft_plot():
    ...
    data = np.loadtxt(
        '../result_for_yasudaetal2022/calculated_expres_detectable_radio_data_of_each_flyby/calculated_all_'+spacecraft_name+'_'+object_name+'_'+str(time_of_flybies)+'_tangential_point_revised.txt')
    H, M, X, Z = Occultation_timing_select(data)
    plt.scatter(X, Z, color='black', linewidth=0.5,
                label="spacecraft position")

    #  凡例用のダミープロット
    plt.plot([-20000, -20000], [-20000, -20000],
             color='red', linewidth=0.5, label="ray path")
    ...
    for i, label in enumerate(annotations):
        plt.annotate(label, (X[i], Z[i]), xytext=(
            X[i]-150, Z[i]-70), annotation_clip=None)
...
def main():
    ...
    plt.title("Maximum density "+highest_plasma +
              "(cm-3), scale height "+plasma_scaleheight+"(km), frequency " + str(round(float(frequency)/1000000, 2))+"(MHz)")
    ...
    spacecraft_plot()
    plt.savefig("../result_for_yasudaetal2022/ray_path_plot_for_paper/maximum density "+highest_plasma +
                "(cm-3), scale height "+plasma_scaleheight+"(km), frequency " + str(round(float(frequency)/1000000, 2))+"(MHz).png", dpi=1000)
    plt.show()
