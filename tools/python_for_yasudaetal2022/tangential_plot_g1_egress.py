...
maximum_subsolar_long = 172.97438273
minimum_subsolar_long = 166.71163854999998
...
def Prepare_Figure(judgement, time_information):
    ...
    ####
    hour_select = np.where(selected_data[:, 0] == 6)

    selected_hour_data = (selected_data[hour_select, :])[0]

    minute_select = np.where(
        (14 < selected_hour_data[:, 1]) & (selected_hour_data[:, 1] < 23))
    ###

    selected_minute_data = (selected_hour_data[minute_select, :])[0]
    # using_frequency_range = [5.5e-1, 6]  # G1 egress
    freq_select = np.where(
        (5.5e-1 < selected_minute_data[:, 2]) & (selected_minute_data[:, 2] < 6))

    selected_freq_data = (selected_minute_data[freq_select, :])[0]
    ...
    plt.savefig(os.path.join('../result_for_yasudaetal2022/plot_tangential_point/', spacecraft_name +
                             '_' + object_name+'_'+str(time_of_flybies)+'_'+radio_type+'_Egress_tangential.png'))
