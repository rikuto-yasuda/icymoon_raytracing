...
plasma_scaleheight = '3e2'  # 単位は(km) 1.5e2/3e2/6e2/9e2
    ...
    for l in range(len(Freq_num)):
        if data[i][2] == Freq_num[l]:
            for j in range(Lowest[l], Highest[l], 2):
                k = str(j)
                data2 = np.genfromtxt("../result_sgepss_2021/"+object_name+"_"+highest_plasma+"_"+plasma_scaleheight+"/ray-P" +
...
A = np.where(res[:, 6] == 0)
res2 = data[A][:]
np.savetxt('../result_sgepss_2021/'+object_name+'_'+highest_plasma+'_'+plasma_scaleheight+'/' +
           object_name+'_'+highest_plasma+'_'+plasma_scaleheight+'_occultaion_data_longver1.txt', res)
np.savetxt('../result_sgepss_2021/'+object_name+'_'+highest_plasma+'_'+plasma_scaleheight+'/' +
           object_name+'_'+highest_plasma+'_'+plasma_scaleheight+'_occultaion_data_longver2.txt', res2)
"""
plt.yscale('log').
...
# %%
data4 = np.loadtxt('../result_sgepss_2021/'+object_name+'_'+highest_plasma+'_'+plasma_scaleheight+'/' +
                   object_name+'_'+highest_plasma+'_'+plasma_scaleheight+'_occultaion_data_longver1.txt')  # 電波源の経度を含む
...
for k in range(len(data4)):
    ...
    if np.abs(galdata[Num][2]+360-data4[k][7]) < np.abs(galdata[Num][2]-data4[k][7]):
...
plt.ylim(0.1,5.0)
plt.title("max density "+highest_plasma+"(/cc) scale height "+plasma_scaleheight+"(km)")
...
fig.savefig(object_name+'_'+highest_plasma+'_'+plasma_scaleheight+'_f-t2.png')
...
grid = ImageGrid(fig, 111, nrows_ncols=(1, 1), axes_pad=0.5, label_mode='L', cbar_location='right', cbar_mode='each', cbar_pad=0.2)
...
plt.imshow(DDF,norm=mpl.colors.LogNorm(),origin='lower',interpolation='nearest',aspect=4,vmin=1E-16, vmax=1E-12,cmap="Spectral_r")
plt.ylim(109,151)
...
pcm = ax.pcolor(xx, yy, DDF, norm=mpl.colors.LogNorm(vmin=1e-16, vmax=1e-12),cmap='Spectral_r')
fig.colorbar(pcm, extend='max')
plt.yscale("log")
plt.ylim(100000,5000000)
"""
