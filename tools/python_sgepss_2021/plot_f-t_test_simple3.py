    ...
    for l in range(len(Freq_num)):
        if data[i][2] == Freq_num[l]:
            for j in range(Lowest[l], Highest[l], 2):
                k = str(j)
                data2 = np.genfromtxt(
                    "../result_sgepss_2021/ganymede_test_simple/ray-Ptest_simple-Mtest_simple-benchmark-LO-Z"+k+"-FR"+Freq_str[l])
                n2 = len(data2)
                aa = 0
...
# %%
data4 = np.loadtxt(
    '../result_sgepss_2021/ganymede_test_simple/ganymede_noreflection_occultaion_data_longver1.txt')  # 電波源の経度を含
...
fig.savefig('ganymede_noreflection_fig.png')
