...
def Judge_occultation(i):
    ...
    if (
        detectable_obsever_position_z > 0 or detectable_obsever_position_x < 0
    ):  # z座標が0以下のものは受かる可能性がレイパスを見る必要もなし
        ...
        #repeat_array1 = np.arange(Lowest[freq], Highest[freq], 100)
        repeat_array1 = np.linspace(Lowest[freq], Highest[freq], 5, dtype=int)
        repeat_array2 = np.arange(Lowest[freq], Highest[freq], 2)
        ...
        for j in repeat_array:
            ...
            if Radio_propagation_route.ndim == 2:
                ...
                if (
                    len(np.intersect1d(detectable_x_index_minus, detectable_z_index))
                    == 0
                ):
                    continue
                ...
                if (
                    ...
                    h = (
                        int(
                            np.intersect1d(
                                detectable_x_index_minus, detectable_z_index
                            )[0]
                        )
                        + 1
                    )
                    ...
                    hight = (
                        Radio_propagation_route[h][3] - detectable_obsever_position_z
                    )
