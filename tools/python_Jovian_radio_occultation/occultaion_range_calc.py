...
def Calc_lowest(l):
    ...
    for i in range(raytrace_lowest_altitude, 0, 100):
        ...
        ray_path = np.genfromtxt(
            ...
            + "-FR"
            + Freq_str[l]
        )  # ray_path[n2][1] x座標   ray_path[n2][3] z座標
...
def Calc_highest(l):
    ...
    for i in range(0, raytrace_highest_altitude, 500):
        ...
        if lower_ray_path.ndim == 2 and higher_ray_path.ndim == 2:
            ...
            if (
                lower_ray_path[n2][1] < x_farthest
                and lower_ray_path[n2][3] < z_farthest
            ):
                print(Freq_str[l], lower_altitude, "error")

            if (
                higher_ray_path[n2][1] < x_farthest
                and higher_ray_path[n2][3] < z_farthest
            ):
