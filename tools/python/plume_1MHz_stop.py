...
# 5e6Hz ライン
F_5e6 = v > 3.53E+11
F_5e6 = F_5e6.astype(np.uint8)

F_5e6_up = np.roll(F_5e6, 1, axis=0)
F_5e6_down = np.roll(F_5e6, -1, axis=0)
F_5e6_right = np.roll(F_5e6, 1, axis=1)
F_5e6_left = np.roll(F_5e6, -1, axis=1)

print_F_5e6 = F_5e6_up + F_5e6_down + F_5e6_right+ F_5e6_left - 4*F_5e6
print_F_5e6 = np.where(print_F_5e6<1, np.nan, 1)
...
plt.imshow(print_F_5e6, cmap='gray', origin='lower', interpolation='nearest', extent=[-1500,1497.5,-750,1497.5])


plt.title("europa_plume_1MHz")
plt.xlabel("x (km)")
plt.ylabel("z (km)")
...
for i in range(100,1301,100):
    n = str(i)
    N = i
    filename = np.genfromtxt("/Users/yasudarikuto/research/icymoon_raytracing/tools/results/for_JpGU_2021/europa_plume_stop_1MHz/ray-Peuropa_plume-Mtest_simple-benchmark-LO-Z"+n+"-FR1e6")
